...
#############################
## in this section we control all
...
for(iter in 1:iM){
  ...
  ##Storing Betas for each estimated model inte monte carlo
  ...
  ##Storing Confidence intervals for all betas in the monte carlo
...
StatsTable <-  as.data.frame(cbind(RB, PB, RMSE, CR, AW, CD))

#setting column  and rownames for the table
colnames(StatsTable) <- c("RB", "PB", "RMSE", "CR", "AW", "R.squared") 
rownames(StatsTable) <- c("Full","CCA","Mean", "Sto", "RF", "PMM") 

## creating latex file to store present the results in a table 
print(xtable(StatsTable, type = "latex"), file = "results.tex", digits = "3")
## look at the results
StatsTable

#########################################
#Plot of confidence intervals of beta in the Monte Carlo
...
#########################################
#Plots of imputed values vs observed values
